...
showtext_auto()
options(modelsummary_latex = "CMU Serif")
```
...
g_pop <- dt_todas[pop %between% quantile(pop, c(.02, .99), na.rm = TRUE)] %>%
  ggplot(aes(x = votos_razao_pp, y = pop, color = as.factor(vitoria_pp))) +
  geom_point(shape = 21, size = 0.5, color = "black") +
  stat_smooth(method = "lm", se = F) +
  scale_color_brewer(palette = "Dark2", direction = -1) +
  geom_vline(xintercept = 0.5, linetype = "dashed") +
  scale_y_continuous(labels = label_number()) +
  theme_minimal(base_family = "CMU Serif") + guides(color = "none") +
  labs(title = "",
       y = "População",
...
g_densidade_demografica <- dt_todas[densidade_demografica %between% quantile(densidade_demografica, c(.02, .99), na.rm = TRUE)] %>%
  ggplot(aes(x = votos_razao_pp, y = densidade_demografica, color = as.factor(vitoria_pp))) +
  geom_point(shape = 21, size = 0.5, color = "black") +
  stat_smooth(method = "lm", se = F) +
  scale_color_brewer(palette = "Dark2", direction = -1) +
  geom_vline(xintercept = 0.5, linetype = "dashed") +
...
g_PIBpc <- dt_todas[PIBpc %between% quantile(PIBpc, c(.02, .99), na.rm = TRUE)] %>%
  ggplot(aes(x = votos_razao_pp, y = PIBpc, color = as.factor(vitoria_pp))) +
  geom_point(shape = 21, size = 0.5, color = "black") +
  stat_smooth(method = "lm", se = F) +
  scale_color_brewer(palette = "Dark2", direction = -1) +
  geom_vline(xintercept = 0.5, linetype = "dashed") +
...
g_taxa_escolarizacao <- dt_todas[taxa_escolarizacao %between% quantile(taxa_escolarizacao, c(.02, .99), na.rm = TRUE)] %>%
  ggplot(aes(x = votos_razao_pp, y = taxa_escolarizacao, color = as.factor(vitoria_pp))) +
  geom_point(shape = 21, size = 0.5, color = "black") +
  stat_smooth(method = "lm", se = F) +
  scale_color_brewer(palette = "Dark2", direction = -1) +
  geom_vline(xintercept = 0.5, linetype = "dashed") +
...
dt_individuais[valor_empenhado_ipca_pc %between% quantile(valor_empenhado_ipca_pc, c(.02, .99), na.rm = TRUE)] %>%
  ggplot(aes(x = votos_razao_pp, y = valor_empenhado_ipca_pc, color = as.factor(vitoria_pp))) +
...
-   As médias de população e Densidade\\ndemográfica são bastante diferentes entre os dois grupos. Vamos olhar para a mediana, que reduz o efeito de outliers:
...
-   Usando a mediana, as médias de população e Densidade\\ndemográfica ficaram bem mais parecidas.

### 2.2.2) Valores empenhados de emendas parlamentares

Vamos calcular a diferença de médias dos valores empenhados per capita dos municípios que tiveram eleições acirradas e elegeram ou não o candidato do PP, o que seria equivalente a ATT dado homogeneidade dos grupos de controle e tratamento.
...
-   Perceba que é uma diferença pequena e negativa, o que não corrobora nossa hipótese.
...
# Duas regressões quadráticas sem controles de município
...
## Completo
nomes_variaveis <- c(
  "votos_razao_pp_centr_sq" = "Razão de votos do PP ao quadrado",
  "votos_razao_pp_centr" = "Razão de votos do PP",
  "vitoria_pp" = "Prefeito do PP"
)
linhas_adicionais <- tribble(~variavel, ~linear, ~linear_controles, ~quad, ~quad_controles,
                  'Forma funcional', "Linear", "Linear", "Quadrática", "Quadrática",
                  'Controles municipais', "Não", "Sim", "Não", "Sim")
modelsummary(list(
    "Linear sem controles" = regs_lineares,
    "Linear com controles" = regs_lineares_controles,
    "Quadrático sem controles" = regs_flexiveis,
    "Quadrático com controles" = regs_flexiveis_controles
  ),
  statistic = NULL,
  stars = T,
  ...
  output = "Resultados/ValorEmpenhadoPerCapitaIndividuaisFinal.png"
)
...
linhas_adicionais <- tribble(~variavel, ~linear, ~linear_controles,
                  'Forma funcional', "Linear", "Linear",
                  'Controles municipais', "Não", "Sim")
modelsummary(list(
    "Linear sem controles" = regs_lineares,
    "Linear com controles" = regs_lineares_controles
  ),
  ...
  coef_omit = 5:12,
  add_rows = linhas_adicionais,
  output = "Resultados/ValorEmpenhadoPerCapitaIndividuaisLinear.png"
)
...
linhas_adicionais <- tribble(~variavel, ~quad, ~quad_controles,
                  'Forma funcional', "Quadrática", "Quadrática",
                  'Controles municipais', "Não", "Sim")
modelsummary(list(
    "Quadrático sem controles" = regs_flexiveis,
    "Quadrático com controles" = regs_flexiveis_controles
  ...
  coef_omit = 7:14,
  add_rows = linhas_adicionais,
  output = "Resultados/ValorEmpenhadoPerCapitaIndividuaisQuad.png"
)
