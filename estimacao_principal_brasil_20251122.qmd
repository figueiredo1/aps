...
# Retirar municípios que tiveram eleições suplementares, identificados pela presença de mais de um candidato eleito na base
municipios_com_mais_de_um_eleito <- dt_eleicoes[resultado == "eleito", .N, by = id_municipio][N>1, unique(id_municipio)]
dt_eleicoes <- dt_eleicoes[!(id_municipio %in% municipios_com_mais_de_um_eleito)]
...
dt_todas[pop %between% quantile(pop, c(.02, .99), na.rm = TRUE)] %>%
  ggplot(aes(x = votos_razao_pp, y = pop)) +
  geom_point(shape = 21) +
  stat_smooth(aes(group = as.factor(vitoria_pp)), method = "loess") +
  geom_vline(xintercept = 0.5, linetype = "dashed") +
  scale_y_continuous(labels = label_number()) +
  theme_minimal() +
  labs(title = "",
       y = "População",
       x = "Razão de votos do PP \n(considerando os 2 candidatos mais votados)")

dt_todas[densidade_demografica %between% quantile(densidade_demografica, c(.02, .99), na.rm = TRUE)] %>%
  ggplot(aes(x = votos_razao_pp, y = densidade_demografica)) +
  geom_point(shape = 21) +
  stat_smooth(aes(group = as.factor(vitoria_pp)), method = "loess") +
  geom_vline(xintercept = 0.5, linetype = "dashed") +
  scale_y_continuous(labels = label_number()) +
...
dt_todas[PIBpc %between% quantile(PIBpc, c(.02, .99), na.rm = TRUE)] %>%
  ggplot(aes(x = votos_razao_pp, y = PIBpc)) +
  geom_point(shape = 21) +
  stat_smooth(aes(group = as.factor(vitoria_pp)), method = "loess") +
  geom_vline(xintercept = 0.5, linetype = "dashed") +
  scale_y_continuous(labels = label_number()) +
...
dt_todas[taxa_escolarizacao %between% quantile(taxa_escolarizacao, c(.02, .99), na.rm = TRUE)] %>%
  ggplot(aes(x = votos_razao_pp, y = taxa_escolarizacao)) +
  geom_point(shape = 21) +
  stat_smooth(aes(group = as.factor(vitoria_pp)), method = "loess") +
  geom_vline(xintercept = 0.5, linetype = "dashed") +
  scale_y_continuous(labels = label_number()) +
...
### 2.2.1) Valor empenhado *per capita* de emendas parlamentares

```{r}
#| label: grafico_valor_empenhado_por_razao_votos_pp

dt_todas[valor_empenhado_ipca_pc %between% quantile(valor_empenhado_ipca_pc, c(.02, .99), na.rm = TRUE)] %>%
  ggplot(aes(x = votos_razao_pp, y = valor_empenhado_ipca_pc)) +
  geom_point(shape = 21) +
  stat_smooth(aes(group = as.factor(vitoria_pp)), method = "loess") +
  geom_vline(xintercept = 0.5, linetype = "dashed") +
  scale_y_continuous(labels = label_number()) +
  theme_minimal() +
  labs(title = "",
       y = "Valor empenhado per capita - Todas as EPs",
       x = "Razão de votos do PP \n(considerando os 2 candidatos mais votados)")
...
# Bancada fica estranho pq há pouquíssimas emendas
dt_bancada[valor_empenhado_ipca_pc %between% quantile(valor_empenhado_ipca_pc, c(.02, .99), na.rm = TRUE)] %>%
  ggplot(aes(x = votos_razao_pp, y = valor_empenhado_ipca_pc)) +
  geom_point(shape = 21) +
  stat_smooth(aes(group = as.factor(vitoria_pp)), method = "loess") +
  geom_vline(xintercept = 0.5, linetype = "dashed") +
  ...
  theme_minimal() +
  labs(title = "",
       y = "Valor empenhado per capita - EPs individuais",
       x = "Razão de votos do PP \n(considerando os 2 candidatos mais votados)")
dt_bancada %>%
  ggplot(aes(x = votos_razao_pp, y = valor_empenhado_ipca_pc)) +
  geom_point(shape = 21) +
...
```{r}
#| label: grafico_barras_medianas_caracteristicas_municipais
...
```{r}
#| label: grafico_barras_medias_emendas_parlamentares
...
-   Note que não faz sentido olhar para medianas porque seriam simplesmente 0 para os dois grupos.
...
Vamos repetir a estimação por região, mas precisamos prestar atenção no número de observações e de tratados.
