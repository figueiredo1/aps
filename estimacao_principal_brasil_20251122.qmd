...
library(extrafont) # para usar fonte do LaTeX
...
ggsave("Graficos/ContinuidadeCaracteristicas.png", width = 10, height = 7, bg = "white")
```
...
dt_individuais[valor_empenhado_ipca_pc %between% quantile(valor_empenhado_ipca_pc, c(.02, .99), na.rm = TRUE)] %>%
  ...
  stat_smooth(method = "lm", se = F) + scale_color_brewer(palette = "Dark2", direction = -1) + scale_color_brewer(palette = "Dark2", direction = -1) +
  geom_vline(xintercept = 0.5, linetype = "dashed") +
  scale_y_continuous(labels = label_number()) +
  theme_minimal() + guides(color = "none") +
  labs(title = "",
