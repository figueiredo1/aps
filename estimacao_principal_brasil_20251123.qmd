...
-   Diferença em relação à versão de 22/11/2025: Escolhi as janelas ótimas segundo o método de Calonico *et al.* em vez de arbitrariamente e coloquei as análises quadráticas como teste de robustez

    -   Note que a janela ótima de modelos lineares e de modelos quadráticos são diferentes!
...
### 2.1.2) Valor empenhado *per capita* de emendas parlamentares
...
### 2.2.0) Obter janela ótima e filtrar bases de dados
...
# 6) Teste de robustez: Modelo quadrático

Vamos repetir as estatísticas e estimação para modelos quadráticos.

## 6.1) Verificação de (des)continuidades

Note que filtramos os municípios para os percentis 2 a 99 de cada característica para evitar distorções causadas por outliers.

### 6.1.1) Características municipais
...
### 6.1.2) Valor empenhado *per capita* de emendas parlamentares
...
## 6.2) Comparação entre tratados e controles

### 6.2.0) Obter janela ótima e filtrar bases de dados
...
### 6.2.1) Características municipais
...
### 6.2.2) Valores empenhados de emendas parlamentares
...
linhas_adicionais <- tribble(~variavel, ~linear, ~linear_controles,
                  'Forma funcional', "Quadrático", "Quadrático",
                  'Controles municipais', "Não", "Sim")
modelsummary(list(
  ...
  coef_omit = 5:12,
  add_rows = linhas_adicionais,
  output = "Resultados/ValorEmpenhadoPerCapitaIndividuaisQuadratico.png"
)
```
