...
\item
  Diferença em relação à versão de 23/11/2025: Retirei as comparações de
  características municipais, pois entendi que não era para filtrar pela
  janela e ver se os municípios são parecidos (para isso, se confia no
  método rs). A ideia das comparações nos artigos é comparar os
  municípios tratados e controles ANTES de filtrar pela janela, para ver
  se eles são sistematicamente diferentes e, portanto, ter quase que uma
  indicação do grau de necessidade do RDD
\end{itemize}
...
\CommentTok{\# de\_para\_emendas\_municipios \textless{}{-} de\_para\_emendas\_municipios[, .(emenda\_numero\_ano, id\_autor\_emenda, id\_municipio)]}
\CommentTok{\# }
\CommentTok{\# \# Salvar base de dados resultante}
...
\NormalTok{de\_para\_emendas\_municipios[, id\_autor\_emenda }\SpecialCharTok{:}\ErrorTok{=} \ConstantTok{NULL}\NormalTok{]}
\end{Highlighting}
\end{Shaded}
...
\subsection{2.1) Comparação das características municipais dos tratados
e
controles}\label{comparauxe7uxe3o-das-caracteruxedsticas-municipais-dos-tratados-e-controles}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Criar dt com médias}
\NormalTok{medias }\OtherTok{\textless{}{-}}\NormalTok{ dt\_individuais[}
\NormalTok{  ,}
\NormalTok{  .(}
    ...
    \AttributeTok{media\_valor\_empenhado =} \FunctionTok{mean}\NormalTok{(valor\_empenhado\_ipca\_pc)}
\NormalTok{  ),}
...
\NormalTok{g1 }\OtherTok{\textless{}{-}}\NormalTok{ medias }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{y =}\NormalTok{ media\_pop, }\AttributeTok{x =} \FunctionTok{as.factor}\NormalTok{(vitoria\_pp), }\AttributeTok{data =}\NormalTok{ )) }\SpecialCharTok{+}
...
\NormalTok{g2 }\OtherTok{\textless{}{-}}\NormalTok{ medias }\SpecialCharTok{\%\textgreater{}\%}
...
\NormalTok{g3 }\OtherTok{\textless{}{-}}\NormalTok{ medias }\SpecialCharTok{\%\textgreater{}\%}
...
\NormalTok{g4 }\OtherTok{\textless{}{-}}\NormalTok{ medias }\SpecialCharTok{\%\textgreater{}\%}
...
\NormalTok{g5 }\OtherTok{\textless{}{-}}\NormalTok{ medias }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{y =}\NormalTok{ media\_valor\_empenhado, }\AttributeTok{x =} \FunctionTok{as.factor}\NormalTok{(vitoria\_pp))) }\SpecialCharTok{+}
  \FunctionTok{geom\_col}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{theme\_minimal}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{labs}\NormalTok{(}\AttributeTok{title =} \StringTok{""}\NormalTok{,}
...
\includegraphics{estimacao_principal_brasil_20251124_files/figure-pdf/grafico_barras_medias_caracteristicas_municipais-1.pdf}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{rm}\NormalTok{(medias, titulo, graficos, g1, g2, g3, g4, g5)}
\end{Highlighting}
\end{Shaded}

\subsection{2.2) Verificação de
...
\includegraphics{estimacao_principal_brasil_20251124_files/figure-pdf/grafico_caracteristicas_municipais_por_razao_votos_pp-1.pdf}

\section{3) Estimação}\label{estimauxe7uxe3o}

\subsection{3.1) Obter janela ótima e filtrar base de
dados}\label{obter-janela-uxf3tima-e-filtrar-base-de-dados}
...
\subsection{3.2) Fazer estimação}\label{fazer-estimauxe7uxe3o}
...
\subsection{3.2) Plotar estimativas}\label{plotar-estimativas}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{regs\_lineares\_controles\_grafico }\OtherTok{\textless{}{-}} \FunctionTok{lm}\NormalTok{(valor\_empenhado\_ipca\_pc }\SpecialCharTok{\textasciitilde{}}\NormalTok{ vitoria\_pp}\SpecialCharTok{*}\NormalTok{votos\_razao\_pp\_centr }\SpecialCharTok{+}\NormalTok{ regiao }\SpecialCharTok{+}\NormalTok{ pop }\SpecialCharTok{+}\NormalTok{ densidade\_demografica }\SpecialCharTok{+}\NormalTok{ PIBpc }\SpecialCharTok{+}\NormalTok{ taxa\_escolarizacao, }\AttributeTok{data =}\NormalTok{ dt\_individuais)}

\CommentTok{\# 1) Filtrar como você vinha fazendo}
\NormalTok{dt\_plot }\OtherTok{\textless{}{-}} \FunctionTok{copy}\NormalTok{(dt\_individuais)[}
\NormalTok{  valor\_empenhado\_ipca\_pc }\SpecialCharTok{\%between\%} \FunctionTok{quantile}\NormalTok{(valor\_empenhado\_ipca\_pc, }\FunctionTok{c}\NormalTok{(.}\DecValTok{02}\NormalTok{, .}\DecValTok{99}\NormalTok{), }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{)}
\NormalTok{]}

\CommentTok{\# 2) Escolher valores de referência para os controles:}
\CommentTok{\# numéricos {-}\textgreater{} média; categórico (regiao) {-}\textgreater{} moda (categoria mais frequente)}
\NormalTok{num\_vars }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\StringTok{"pop"}\NormalTok{, }\StringTok{"densidade\_demografica"}\NormalTok{, }\StringTok{"PIBpc"}\NormalTok{, }\StringTok{"taxa\_escolarizacao"}\NormalTok{)}
\NormalTok{cat\_var  }\OtherTok{\textless{}{-}} \StringTok{"regiao"}

\NormalTok{refs }\OtherTok{\textless{}{-}} \FunctionTok{list}\NormalTok{()}
\ControlFlowTok{for}\NormalTok{ (v }\ControlFlowTok{in}\NormalTok{ num\_vars) refs[[v]] }\OtherTok{\textless{}{-}} \FunctionTok{mean}\NormalTok{(dt\_plot[[v]], }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{)}
\CommentTok{\# modo para regiao}
\NormalTok{refs[[cat\_var]] }\OtherTok{\textless{}{-}}\NormalTok{ dt\_plot[, .N, by }\OtherTok{=}\NormalTok{ regiao][}\FunctionTok{order}\NormalTok{(}\SpecialCharTok{{-}}\NormalTok{N)][}\DecValTok{1}\NormalTok{, regiao]}

\CommentTok{\# 3) Grid QUEBRADO por lado do cutoff}

\CommentTok{\# lado perdedor (0): só valores \textless{} 0}
\NormalTok{x\_left }\OtherTok{\textless{}{-}} \FunctionTok{seq}\NormalTok{(}\FunctionTok{min}\NormalTok{(dt\_plot[vitoria\_pp }\SpecialCharTok{==} \DecValTok{0}\NormalTok{]}\SpecialCharTok{$}\NormalTok{votos\_razao\_pp\_centr, }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{),}
              \DecValTok{0}\NormalTok{,}
              \AttributeTok{length.out =} \DecValTok{200}\NormalTok{)}

\NormalTok{grid\_left }\OtherTok{\textless{}{-}} \FunctionTok{data.table}\NormalTok{(}
  \AttributeTok{vitoria\_pp =} \DecValTok{0}\NormalTok{,}
  \AttributeTok{votos\_razao\_pp\_centr =}\NormalTok{ x\_left}
\NormalTok{)}

\CommentTok{\# lado vencedor (1): só valores \textgreater{} 0}
\NormalTok{x\_right }\OtherTok{\textless{}{-}} \FunctionTok{seq}\NormalTok{(}\DecValTok{0}\NormalTok{,}
               \FunctionTok{max}\NormalTok{(dt\_plot[vitoria\_pp }\SpecialCharTok{==} \DecValTok{1}\NormalTok{]}\SpecialCharTok{$}\NormalTok{votos\_razao\_pp\_centr, }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{),}
               \AttributeTok{length.out =} \DecValTok{200}\NormalTok{)}

\NormalTok{grid\_right }\OtherTok{\textless{}{-}} \FunctionTok{data.table}\NormalTok{(}
  \AttributeTok{vitoria\_pp =} \DecValTok{1}\NormalTok{,}
  \AttributeTok{votos\_razao\_pp\_centr =}\NormalTok{ x\_right}
\NormalTok{)}

\CommentTok{\# adicionar controles aos dois grids}
\ControlFlowTok{for}\NormalTok{(v }\ControlFlowTok{in}\NormalTok{ num\_vars)\{}
\NormalTok{  grid\_left[[v]]  }\OtherTok{\textless{}{-}}\NormalTok{ refs[[v]]}
\NormalTok{  grid\_right[[v]] }\OtherTok{\textless{}{-}}\NormalTok{ refs[[v]]}
\NormalTok{\}}
\NormalTok{grid\_left[[cat\_var]]  }\OtherTok{\textless{}{-}}\NormalTok{ refs[[cat\_var]]}
\NormalTok{grid\_right[[cat\_var]] }\OtherTok{\textless{}{-}}\NormalTok{ refs[[cat\_var]]}

\CommentTok{\# juntar resultados}
\NormalTok{newdata\_grid }\OtherTok{\textless{}{-}} \FunctionTok{rbind}\NormalTok{(grid\_left, grid\_right)}

\CommentTok{\# certifique{-}se que fatores no newdata têm os mesmos níveis que no modelo/data}
\CommentTok{\# (se regiao for factor)}
\ControlFlowTok{if}\NormalTok{ (}\FunctionTok{is.factor}\NormalTok{(dt\_individuais}\SpecialCharTok{$}\NormalTok{regiao)) \{}
\NormalTok{  newdata\_grid[, regiao }\SpecialCharTok{:}\ErrorTok{=} \FunctionTok{factor}\NormalTok{(regiao, }\AttributeTok{levels =} \FunctionTok{levels}\NormalTok{(dt\_individuais}\SpecialCharTok{$}\NormalTok{regiao))]}
\NormalTok{\}}
\ControlFlowTok{if}\NormalTok{ (}\FunctionTok{is.factor}\NormalTok{(dt\_individuais}\SpecialCharTok{$}\NormalTok{vitoria\_pp)) \{}
\NormalTok{  newdata\_grid[, vitoria\_pp }\SpecialCharTok{:}\ErrorTok{=} \FunctionTok{factor}\NormalTok{(vitoria\_pp, }\AttributeTok{levels =} \FunctionTok{levels}\NormalTok{(dt\_individuais}\SpecialCharTok{$}\NormalTok{vitoria\_pp))]}
\NormalTok{\}}

\CommentTok{\# 4) Prever usando o lm que você rodou (gera fit, lwr, upr)}
\NormalTok{preds }\OtherTok{\textless{}{-}} \FunctionTok{predict}\NormalTok{(regs\_lineares\_controles\_grafico,}
...
\NormalTok{newdata\_grid[, }\StringTok{\textasciigrave{}}\AttributeTok{:=}\StringTok{\textasciigrave{}}\NormalTok{(}
...
\CommentTok{\# 5) Plot: pontos reais + linhas preditas (uma por vitoria\_pp) + ribbon}
\FunctionTok{ggplot}\NormalTok{() }\SpecialCharTok{+}
  \CommentTok{\# pontos reais (opcional, com alpha pra não poluir)}
  \FunctionTok{geom\_point}\NormalTok{(}\AttributeTok{data =}\NormalTok{ dt\_plot, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ votos\_razao\_pp\_centr, }\AttributeTok{y =}\NormalTok{ valor\_empenhado\_ipca\_pc,}
                                 \AttributeTok{color =} \FunctionTok{as.factor}\NormalTok{(vitoria\_pp)),}
             \AttributeTok{shape =} \DecValTok{21}\NormalTok{, }\AttributeTok{color =} \StringTok{"black"}\NormalTok{, }\AttributeTok{alpha =} \FloatTok{0.6}\NormalTok{, }\AttributeTok{size =} \DecValTok{1}\NormalTok{) }\SpecialCharTok{+}
  \CommentTok{\# linha predita}
  \FunctionTok{geom\_line}\NormalTok{(}\AttributeTok{data =}\NormalTok{ newdata\_grid, }
            \FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ votos\_razao\_pp\_centr, }\AttributeTok{y =}\NormalTok{ fit, }\AttributeTok{color =} \FunctionTok{as.factor}\NormalTok{(vitoria\_pp)),}
  ...
  \FunctionTok{theme\_minimal}\NormalTok{() }\SpecialCharTok{+} \FunctionTok{guides}\NormalTok{(}\AttributeTok{color =} \StringTok{"none"}\NormalTok{, }\AttributeTok{fill =} \StringTok{"none"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{labs}\NormalTok{(}\AttributeTok{y =} \StringTok{"Valor empenhado per capita"}\NormalTok{, }\AttributeTok{x =} \StringTok{"Margem de vitória do PP"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\includegraphics{estimacao_principal_brasil_20251124_files/figure-pdf/grafico_valor_empenhado_por_razao_votos_pp_linear_com_controles-1.pdf}
...
\subsubsection{6.3.1) Obter janela ótima e filtrar bases de
...
\item
  Note que nenhum município com eleições acirradas recebeu emendas de
  relator e somente 9 receberam emendas de bancada, então vamos analisar
  somente emendas individuais.
\end{itemize}

\subsubsection{6.3.1) Estimar}\label{estimar}
...
\subsubsection{6.3.2) Plotar estimativas}\label{plotar-estimativas-1}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# 1) Filtrar dados para o gráfico}
\NormalTok{dt\_plot }\OtherTok{\textless{}{-}} \FunctionTok{copy}\NormalTok{(dt\_individuais)[}
\NormalTok{  valor\_empenhado\_ipca\_pc }\SpecialCharTok{\%between\%} \FunctionTok{quantile}\NormalTok{(valor\_empenhado\_ipca\_pc, }\FunctionTok{c}\NormalTok{(.}\DecValTok{02}\NormalTok{, .}\DecValTok{99}\NormalTok{), }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{)}
\NormalTok{]}

\CommentTok{\# 2) Rodar lm apenas para gerar predições}
\NormalTok{regs\_grafico }\OtherTok{\textless{}{-}} \FunctionTok{lm}\NormalTok{(valor\_empenhado\_ipca\_pc }\SpecialCharTok{\textasciitilde{}}\NormalTok{ vitoria\_pp}\SpecialCharTok{*}\NormalTok{votos\_razao\_pp\_centr,}
                   \AttributeTok{data =}\NormalTok{ dt\_plot)}

\CommentTok{\# 3) Criar grids separados por lado do cutoff}

\CommentTok{\# esquerda (vitoria\_pp = 0)}
\NormalTok{x\_left }\OtherTok{\textless{}{-}} \FunctionTok{seq}\NormalTok{(}\FunctionTok{min}\NormalTok{(dt\_plot[vitoria\_pp }\SpecialCharTok{==} \DecValTok{0}\NormalTok{]}\SpecialCharTok{$}\NormalTok{votos\_razao\_pp\_centr, }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{),}
              \DecValTok{0}\NormalTok{,}
              \AttributeTok{length.out =} \DecValTok{200}\NormalTok{)}
\NormalTok{grid\_left }\OtherTok{\textless{}{-}} \FunctionTok{data.table}\NormalTok{(}
...
\CommentTok{\# direita (vitoria\_pp = 1)}
\NormalTok{x\_right }\OtherTok{\textless{}{-}} \FunctionTok{seq}\NormalTok{(}\DecValTok{0}\NormalTok{,}
               \FunctionTok{max}\NormalTok{(dt\_plot[vitoria\_pp }\SpecialCharTok{==} \DecValTok{1}\NormalTok{]}\SpecialCharTok{$}\NormalTok{votos\_razao\_pp\_centr, }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{),}
               \AttributeTok{length.out =} \DecValTok{200}\NormalTok{)}
\NormalTok{grid\_right }\OtherTok{\textless{}{-}} \FunctionTok{data.table}\NormalTok{(}
...
\CommentTok{\# 4) Previsão das linhas}
\NormalTok{newdata\_grid[, fit }\SpecialCharTok{:}\ErrorTok{=} \FunctionTok{predict}\NormalTok{(regs\_grafico, }\AttributeTok{newdata =}\NormalTok{ newdata\_grid)]}

\CommentTok{\# 5) Plot}
\FunctionTok{ggplot}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{geom\_point}\NormalTok{(}\AttributeTok{data =}\NormalTok{ dt\_plot,}
             \FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ votos\_razao\_pp\_centr, }\AttributeTok{y =}\NormalTok{ valor\_empenhado\_ipca\_pc,}
                 \AttributeTok{color =} \FunctionTok{as.factor}\NormalTok{(vitoria\_pp)),}
             \AttributeTok{shape =} \DecValTok{21}\NormalTok{, }\AttributeTok{color =} \StringTok{"black"}\NormalTok{, }\AttributeTok{alpha =} \FloatTok{0.6}\NormalTok{, }\AttributeTok{size =} \DecValTok{1}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{geom\_line}\NormalTok{(}\AttributeTok{data =}\NormalTok{ newdata\_grid,}
            \FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ votos\_razao\_pp\_centr, }\AttributeTok{y =}\NormalTok{ fit, }\AttributeTok{color =} \FunctionTok{as.factor}\NormalTok{(vitoria\_pp)),}
  ...
  \FunctionTok{geom\_vline}\NormalTok{(}\AttributeTok{xintercept =} \DecValTok{0}\NormalTok{, }\AttributeTok{linetype =} \StringTok{"dashed"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{scale\_color\_brewer}\NormalTok{(}\AttributeTok{palette =} \StringTok{"Dark2"}\NormalTok{, }\AttributeTok{direction =} \SpecialCharTok{{-}}\DecValTok{1}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{scale\_y\_continuous}\NormalTok{(}\AttributeTok{labels =}\NormalTok{ scales}\SpecialCharTok{::}\FunctionTok{label\_currency}\NormalTok{(}\AttributeTok{prefix =} \StringTok{"R$"}\NormalTok{)) }\SpecialCharTok{+}
  \FunctionTok{scale\_x\_continuous}\NormalTok{(}\AttributeTok{labels =}\NormalTok{ scales}\SpecialCharTok{::}\FunctionTok{label\_percent}\NormalTok{()) }\SpecialCharTok{+}
  \FunctionTok{theme\_minimal}\NormalTok{() }\SpecialCharTok{+} \FunctionTok{guides}\NormalTok{(}\AttributeTok{color =} \StringTok{"none"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{labs}\NormalTok{(}\AttributeTok{x =} \StringTok{"Margem de vitória do PP"}\NormalTok{, }\AttributeTok{y =} \StringTok{"Valor empenhado per capita"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\includegraphics{estimacao_principal_brasil_20251124_files/figure-pdf/grafico_valor_empenhado_por_razao_votos_pp_linear_sem_controles-1.pdf}
...
\includegraphics{estimacao_principal_brasil_20251124_files/figure-pdf/grafico_caracteristicas_municipais_por_razao_votos_pp_quad-1.pdf}

\subsection{7.2) Estimação}\label{estimauxe7uxe3o-2}

\subsubsection{7.2.1) Obter janela ótima e filtrar bases de
...
\item
  Note que nenhum município com eleições acirradas recebeu emendas de
  relator e somente 12 receberam emendas de bancada, então vamos
  analisar somente emendas individuais.
\end{itemize}

\subsubsection{7.2.2) Estimar}\label{estimar-1}
...
\subsubsection{7.2.3) Plotar estimativas}\label{plotar-estimativas-2}
...
\CommentTok{\# 4) Adicionar quadrático e controles}
\NormalTok{newdata\_grid[, votos\_razao\_pp\_centr\_sq }\SpecialCharTok{:}\ErrorTok{=}\NormalTok{ votos\_razao\_pp\_centr}\SpecialCharTok{\^{}}\DecValTok{2}\NormalTok{]}

\ControlFlowTok{for}\NormalTok{(v }\ControlFlowTok{in}\NormalTok{ num\_vars)\{}
\NormalTok{  newdata\_grid[[v]] }\OtherTok{\textless{}{-}}\NormalTok{ refs[[v]]}
\NormalTok{\}}
\NormalTok{newdata\_grid[[cat\_var]] }\OtherTok{\textless{}{-}}\NormalTok{ refs[[cat\_var]]}

\CommentTok{\# Certificar que fatores têm os mesmos níveis}
\ControlFlowTok{if}\NormalTok{(}\FunctionTok{is.factor}\NormalTok{(dt\_individuais}\SpecialCharTok{$}\NormalTok{regiao)) \{}
\NormalTok{  newdata\_grid[, regiao }\SpecialCharTok{:}\ErrorTok{=} \FunctionTok{factor}\NormalTok{(regiao, }\AttributeTok{levels =} \FunctionTok{levels}\NormalTok{(dt\_individuais}\SpecialCharTok{$}\NormalTok{regiao))]}
\NormalTok{\}}
\ControlFlowTok{if}\NormalTok{(}\FunctionTok{is.factor}\NormalTok{(dt\_individuais}\SpecialCharTok{$}\NormalTok{vitoria\_pp)) \{}
...
\CommentTok{\# 5) Criar LM apenas para o gráfico}
\NormalTok{regs\_quad\_grafico }\OtherTok{\textless{}{-}} \FunctionTok{lm}\NormalTok{(}
\NormalTok{  valor\_empenhado\_ipca\_pc }\SpecialCharTok{\textasciitilde{}}\NormalTok{ vitoria\_pp}\SpecialCharTok{*}\NormalTok{votos\_razao\_pp\_centr }\SpecialCharTok{+}\NormalTok{ vitoria\_pp}\SpecialCharTok{*}\NormalTok{votos\_razao\_pp\_centr\_sq }\SpecialCharTok{+}
\NormalTok{    regiao }\SpecialCharTok{+}\NormalTok{ pop }\SpecialCharTok{+}\NormalTok{ densidade\_demografica }\SpecialCharTok{+}\NormalTok{ PIBpc }\SpecialCharTok{+}\NormalTok{ taxa\_escolarizacao,}
  \AttributeTok{data =}\NormalTok{ dt\_plot}
\NormalTok{)}

\CommentTok{\# 6) Previsão}
\NormalTok{newdata\_grid[, fit }\SpecialCharTok{:}\ErrorTok{=} \FunctionTok{predict}\NormalTok{(regs\_quad\_grafico, }\AttributeTok{newdata =}\NormalTok{ newdata\_grid)]}

\CommentTok{\# 7) Plot final}
\FunctionTok{ggplot}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{geom\_point}\NormalTok{(}\AttributeTok{data =}\NormalTok{ dt\_plot,}
             \FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ votos\_razao\_pp\_centr, }\AttributeTok{y =}\NormalTok{ valor\_empenhado\_ipca\_pc,}
                 \AttributeTok{color =} \FunctionTok{as.factor}\NormalTok{(vitoria\_pp)),}
...
\includegraphics{estimacao_principal_brasil_20251124_files/figure-pdf/grafico_valor_empenhado_por_razao_votos_pp_quad_com_controles-1.pdf}
