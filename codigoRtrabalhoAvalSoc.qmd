...
# Tvz criar dt com medianas se médias não forem favoráveis

# Plotar médias de quem venceu vs. não venceu
dt_municipios_eleicoes_medias %>%
  ggplot(aes(x = media_IDHm, fill = as.factor(vitoria_pp))) +
  geom_bar() +
  theme_minimal()

dt_municipios_eleicoes_medias %>%
  ggplot(aes(x = media_PIBpc, fill = as.factor(vitoria_pp))) +
  geom_bar() +
  theme_minimal()

dt_municipios_eleicoes_medias %>%
  ggplot(aes(x = media_percentual_renda_pc_meio_sm, fill = as.factor(vitoria_pp))) +
  geom_bar() +
  theme_minimal()

dt_municipios_eleicoes_medias %>%
  ggplot(aes(x = media_taxa_escolarizacao, fill = as.factor(vitoria_pp))) +
  geom_bar() +
  theme_minimal()

dt_municipios_eleicoes_medias %>%
  ggplot(aes(x = media_pop, fill = as.factor(vitoria_pp))) +
  geom_bar() +
  theme_minimal()

dt_municipios_eleicoes_medias %>%
  ggplot(aes(x = media_densidade_demografica, fill = as.factor(vitoria_pp))) +
  geom_bar() +
  theme_minimal()

# Mostrar quais diferenças são estatisticamente significantes

```

-   Plotar gráfico (x = margem de votos do candidato do PP, y = valor de emendas recebido em AAAA) em busca da descontinuidade

```{r}

# ALGO COMO
# dt %>%
#   ggplot(aes(x = razao_votos_pp, y = valor_empenhado)) +
#   geom_line()
